...
For debugging purposes, AI-based software like ChatGPT and CS50.ai were utilized. 

NOTE: the following README description is fairly large due to the sheer scale of this project; going into detail of my intention and the functionality behind each relevant backend and frontend file required an intense amount of time and effort to complete. Have Fun Reading!
...
### ContentView.swift
ContentView is the SwiftUI file that defines the structure of Pokedata by using TabView for navigation between four primary tabs: Cards, Search, Collection, and Profile. The app incorporates various data models (e.g., Pokemon, PokemonCard, UserInfo) and observable objects (cards, series) to manage Pokémon-related and user-specific data. Each tab displays a distinct view, such as search functionalities, card collections, and user profiles, with state variables tracking user interactions and app behavior, like login status, selected Pokémon/cards, and active views. The app also supports future enhancements, such as camera integration for Pokemon card detection and identification, essentially the ability to query for cards using the camera instead of text input. Other enhancements could be the addition of a history view, where users can keep track of what cards and pokemon they have already viewed. These possible enhancements are indicated by commented-out placeholders within certain frontend files.
#### Key Features:
